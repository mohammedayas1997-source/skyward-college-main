  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      if (docSnap.exists()) {
        ...
        // Role Protection: Tabbatar mutum ba zai iya shiga dashboard din da ba nasa ba
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[1, 2, 3].map(i => (
            <div key={i} className="p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 flex flex-col justify-end min-h-[160px]">
              ...
              <span className="text-[#002147] font-black text-[10px] uppercase tracking-widest">Portal Module</span>
            </div>
          ))}
        </div>
      </div>
