...
import { Phone, Mail, Lock, Headphones, BookOpen, Menu, X, ChevronDown, Newspaper } from "lucide-react"; 
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <nav className="hidden lg:block">
            <ul className="flex gap-6 text-[10px] font-black text-[#002147] uppercase items-center">
              ...
              {/* SABON LATEST NEWS ICON A SAMA */}
              <li>
                <Link to="/news" className="hover:text-red-600 flex items-center gap-1 text-black-600">
              ...
              <li className="relative group hover:text-red-600 cursor-pointer py-2">
                ...
                <ul className="absolute left-0 top-full hidden group-hover:block bg-white shadow-2xl border-t-4 border-red-600 w-72 z-[100] py-2">
                  {[{ name: "The Rector", path: "/about/rector" }, { name: "The Registrar", path: "/about/registrar" }, { name: "Academic Planning", path: "/about/dir-planning" }].map((item) => (
                    <li key={item.path} className="px-4 py-2 hover:bg-slate-100 border-b border-slate-50 text-slate-700">
              ...
              <li className="relative group hover:text-red-600 cursor-pointer py-2">
                ...
                <ul className="absolute right-0 top-full hidden group-hover:block bg-white shadow-2xl border-t-4 border-red-600 w-64 z-[100] py-2 text-slate-700">
                  <li className="px-4 py-2 hover:bg-slate-100 border-b border-slate-50 flex items-center gap-2">
                    ...
                    <Link to="/portal/student-login">Student Login</Link>
                  </li>
                  <li className="px-4 py-2 hover:bg-slate-100 border-b border-slate-50 flex items-center gap-2">
                    <Lock size={15} className="text-red-600" />
        ...
        <div className="p-6 flex flex-col h-full text-white">
          ...
          <nav className="flex-grow">
            <ul className="space-y-6 text-sm font-black uppercase tracking-widest">
              ...
              {/* SABON LATEST NEWS ICON A MOBILE SIDEBAR */}
              <li>
                <Link onClick={() => setIsMenuOpen(false)} to="/news" className="flex items-center gap-2 py-2 border-b border-white/10 text-red-400">
              ...
              <li><Link onClick={() => setIsMenuOpen(false)} to="/portal/student-login" className="block py-2 border-b border-white/10">Student Portal</Link></li>
