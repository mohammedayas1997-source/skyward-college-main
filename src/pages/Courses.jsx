...
import { X, CheckCircle, Briefcase, ArrowRight, Wallet, Image as ImageIcon } from "lucide-react";

const Courses = () => {
  const [selectedCourse, setSelectedCourse] = useState(null);

  // Wannan shi ne asalin hotunan da na zaba maka (No Firebase needed)
  const courseData = [
    {
      id: 1,
          ...
          {courseData.map((course) => (
            <div key={course.id} className="bg-white rounded-[2.5rem] overflow-hidden shadow-lg border border-slate-100 hover:shadow-2xl transition-all group flex flex-col h-full hover:-translate-y-2 duration-500">
              <div className="h-64 w-full overflow-hidden relative">
                ...
                <div className="absolute bottom-5 right-5 bg-white/95 px-4 py-2 rounded-2xl shadow-xl">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8 bg-[#001524]/95 backdrop-blur-md">
          <div className="bg-white w-full max-w-5xl max-h-[90vh] rounded-[3rem] overflow-hidden relative shadow-2xl flex flex-col md:flex-row">
            ...
            <div className="w-full md:w-1/2 p-10 md:p-14 overflow-y-auto">
              ...
              <div className="bg-red-50 px-5 py-3 rounded-2xl border border-red-100 mb-8 inline-block"><p className="text-red-600 font-black text-sm leading-none">Course Fee: {selectedCourse.fee}</p></div>
              ...
              <button className="w-full mt-12 bg-[#002147] text-white font-black py-5 rounded-[2rem] uppercase text-[11px] tracking-widest hover:bg-red-600 transition-all shadow-xl">Apply Now</button>
            </div>
          </div>
        </div>
      )}
